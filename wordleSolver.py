...
def calculate_letter_probability(word_list, word_length):
    """
    Calculate the probability of each letter appearing at each position in the word.
...
def main(word_length=5, word_list_length=900):
    ...
    word_list = random.sample(word_list, k=word_list_length)
    ...
    guesses = 0
    print(f"{'Guess':<10} | {'List Size':<10} | {'Entropy':<10}")
    print("-" * 35)
    while True:
        guesses += 1
        guessed_word = guess_highest_entropy_word(word_entropy_dict)
        print(f"{guessed_word:<10} | {len(word_list):<10} | {word_entropy_dict[guessed_word]:<10.4f}")
        if guessed_word == target_word:
            print(f"Correct! The word was '{target_word}' in {guesses} guesses.")
            break
...
total_guesses = 0
n_runs = 100  # Reduced for demonstration; adjust as needed
for i in range(n_runs):
    guesses = main()
    total_guesses += guesses
    print(f"Run {i+1}: {guesses} guesses. Average after {i+1} runs: {total_guesses / (i + 1):.2f}\n")
